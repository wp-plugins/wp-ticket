...
function wp_ticket_com_set_submit_tickets() {
	...
	$obj->set_rule(array(
		...
		'email' => array(
			'error',
			__('Email: Please enter a valid email address')
		) ,
	));
	//text
...
function wp_ticket_com_process_submit_tickets() {
	...
	if ($show_form == 1) {
		return emd_submit_php_form('submit_tickets', 'wp_ticket_com', 'emd_ticket', 'publish', 'Thanks for your submission.', 'There has been an error when submitting your entry. Please contact the site administrator.', 0, 1);
	} else {
