...
function emd_limit_by($pids = Array() , $app, $check_ptype, $where = 'frontend', $prev_next = 0) {
	...
	if (!empty($limitby_auth_caps[$check_ptype])) {
		$pids = emd_get_author_pids($pids, $limitby_auth_caps[$check_ptype], $check_ptype, $where, $prev_next);
	}
 ...
 * @return array $pids accessible post ids
 */
function emd_get_author_pids($pids, $rel_arr, $ptype, $where, $prev_next) {
	global $user_ID, $wpdb, $filter;
 ...
 * @return object $query
 */
function emd_apm_filter($app, $query) {
	...
	if (!isset($query->query_vars['apm_custom']) && ($filter == 1 || !empty($query->query_vars['post__in']))) {
	...
	if (!empty($_GET['author']) && !isset($query->query_vars['apm_filter']) && !isset($query->query_vars['apm_custom'])) {
...
function emd_filter_views($view) {
	...
	if (empty($relposts) && !$wp_query->query_vars['apm_filter']) {
		return $view;
	}
	if (isset($wp_query->query_vars['apm_filter']) && $wp_query->query_vars['apm_filter'] == 1) {
		return array();
	}
	...
	if (!empty($relposts) && !isset($wp_query->query_vars['apm_filter'])) {
