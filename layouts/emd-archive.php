...
<?php 
	$has_sidebar = apply_filters( 'emd_show_temp_sidebar', 'right', 'wp_ticket_com', 'archive');
	if($has_sidebar ==  'left'){
		do_action( 'emd_sidebar', 'wp-ticket-com' );
	}
	if($has_sidebar == 'full'){
?>
...
<?php
	...
	while ( have_posts() ) : the_post(); ?>
			...
			<?php emd_get_template_part('wp-ticket-com', 'archive', str_replace("_","-",$post->post_type)); ?>
			</div>
                <?php endwhile; // end of the loop. ?>
<?php	$has_navigation = apply_filters( 'emd_show_temp_navigation', true, 'wp_ticket_com', 'archive');
	if($has_navigation){
		global $wp_query;
