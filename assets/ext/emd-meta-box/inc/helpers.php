	...
	class EMD_MB_Helper
	{
		...
		static function file_info( $id )
		{
			...
			return array(
				...
				'name'  => basename( $path ),
				'path'  => $path,
				'url'   => str_replace('http:','',wp_get_attachment_url( $id )),
		...
		static function image_info( $id, $args = array() )
		{
			...
			return array(
				...
				'name'        => basename( $path ),
				'path'        => $path,
				'url'         => str_replace('http:','',$img_src[0]),
				'width'       => $img_src[1],
				'height'      => $img_src[2],
				'full_url'    => str_replace('http:','',wp_get_attachment_url( $id )),
 ...
 * @return mixed
 */
function emd_mb_meta( $key, $args = array(), $post_id = null )
{
	return EMD_MB_Helper::meta( $key, $args, $post_id );
}
