 ...
 *  @package    Controls
 */
class Zebra_Form_Selectadv extends Zebra_Form_Control
{
     ...
     *  @return void
     */
    function __construct($id, $default = '', $attributes = '', $default_other = '',$data_options='{"allowClear":true,"placeholder":"Select a value","placeholderOption":"first"}')
    {
    ...
    function add_options($options, $overwrite = false)
    {
        ...
        if (is_array($options)) {
            ...
            if (empty($attributes['options']) && $overwrite === false)
    ...
    function toHTML()
    {
        ...
        //return '<select '. $this->_render_attributes() . ' data-options="{"allowClear":true,"width":"resolve","placeholder":"Select a value"}">' . $optContent . '</select>';
