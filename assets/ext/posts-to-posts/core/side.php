...
class P2P_Side_Post extends P2P_Side {
	...
	function translate_qv( $qv ) {
		...
		//EMD: 06/17/2016 , wp query doesn't process both include and exclude 	
		if(!empty($qv["p2p:exclude"]) && !empty($qv["p2p:include"])){
                        $new_include = array_diff($qv["p2p:include"],$qv["p2p:exclude"]);
                        unset($qv["p2p:exclude"]);
                        if(empty($new_include)){
                                $new_include = Array('0');
                        }
                        $qv["p2p:include"] = $new_include;
                }
	...
	function do_query( $args ) {
		//EMD: 12/23/2015 	
		//when there is no orderby, the default orderby is date desc, when data is imported dates are same
		// and orderby date doesn't work and show dupes, therefore changed orderby to ID DESC
